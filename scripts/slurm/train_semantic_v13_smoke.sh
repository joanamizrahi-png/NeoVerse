#!/usr/bin/env bash
#SBATCH --job-name=train-sem-v13smoke
...
mkdir -p /scratch/m000204-pm06b/joana/runs/train_semantic_v13smoke_bits
...
grep -q "semantic_analog_bits: true" training/configs/train_semantic_v13smoke_bits.yaml && grep -q "snr_gamma: 5.0" training/configs/train_semantic_v13smoke_bits.yaml && grep -q "semantic_ce_sigma_max: 1.0" training/configs/train_semantic_v13smoke_bits.yaml \
    || { echo "[sanity] FATAL: v10 config missing the v13 analog-bits knobs"; exit 1; }

python train.py training/configs/train_semantic_v13smoke_bits.yaml

echo "==> v13 bits smoke done; checkpoints in /scratch/m000204-pm06b/joana/runs/train_semantic_v13smoke_bits/"
