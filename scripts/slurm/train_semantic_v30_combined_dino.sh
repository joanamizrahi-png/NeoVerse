#!/usr/bin/env bash
#SBATCH --job-name=train-sem-v30_combined_dino
...
#SBATCH --output=/scratch/m000204-pm06b/joana/slurm-train-sem-v30_combined_dino-%j.out
#SBATCH --error=/scratch/m000204-pm06b/joana/slurm-train-sem-v30_combined_dino-%j.err

# v30_combined_dino: campus-only semantics (SANPO, no RUGD, ~2x clips). See the config
# header for why. Needs the v26 dataset build (job 459173) to have finished.

set -euo pipefail
mkdir -p /scratch/m000204-pm06b/joana/runs/train_semantic_v30_combined_dino
...
grep -q "sanpo_v29" training/configs/train_semantic_v30_combined_dino.yaml \
    || { echo "[sanity] FATAL: config not pointing at the v26 roots"; exit 1; }

python train.py training/configs/train_semantic_v30_combined_dino.yaml
echo "==> v30_combined_dino done; checkpoints in /scratch/m000204-pm06b/joana/runs/train_semantic_v30_combined_dino/"
